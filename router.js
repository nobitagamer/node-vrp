...
function Router (opts) {
  if (!Array.isArray(opts.customers)) { throw new Error('customers must be Array') }
  if (!Array.isArray(opts.distances) || !Array.isArray(opts.distances[0])) { throw new Error('distances must be Array of Arrays') }
...
//
Router.prototype.reduceTemperature = function () {
...
Router.prototype.isCooled = function () {
  ...
  if (maxLen > 0) {
    isAcceptable = this.solution.every(function (route) {
      ...
      if (len < maxLen) { return true } else { return false }
    })
  }

  if (isStable) { this.log('system is stable (> %s non-improving iterations)', this.opts.stability) }
  if (isCool) { this.log('system is cooled (< %s temperature)', this.opts.coolTemp) }
  if (isAcceptable) { this.log('system routes are acceptable length (< %s)', this.opts.maxRouteLength) }

  if (isStable && isCool && isAcceptable) { return true } else { return false }
}
...
Router.prototype.log = function () {
  if (this.opts.verbose) { console.log.apply(console, [].slice.call(arguments)) }
}
